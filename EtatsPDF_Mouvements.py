...
class EtatPDFMouvements:
    ...
    def _get_societe_info(self):
        ...
        try:
            ...
            cur.execute("""
                SELECT 
                    snomentreprise, sadresseentreprise, scontactentreprise,
                    sniventreprise, sstatentreprise
                FROM tb_entreprise LIMIT 1
            """)
            row = cur.fetchone()
            if row:
                return {
                    'nomsociete': row[0] or 'IJEERY',
                    'adressesociete': row[1] or 'Adresse Non Configurée',
                    'contactsociete': row[2] or 'Contact: Non Configuré',
                    'nifsociete': row[3] or 'NIF: Non Configuré',
                    'statsociete': row[4] or 'STAT: Non Configurée'
                }
        except Exception as e:
    ...
    def _build_pdf_a5(self, output_path, titre_entete, reference, date_operation, 
        ...
        main_title = Paragraph(
            "Ankino amin'ny Jehovah ny asanaka dia ho lavorary izay kasaina",
            ParagraphStyle(
                'MainTitle',
        ...
        title_table.setStyle(TableStyle([
            ('BORDER', (0, 0), (-1, -1), 1.5, self.COLOR_HEADER),
        ...
        company_details = Paragraph(
            f"Adresse : {societe['adressesociete']}<br/>"
            f"Ville : Madame<br/>"
            f"Contact : {societe['contactsociete']}<br/>"
            f"NIF : {societe['nifsociete']}<br/>"
            f"STAT : {societe['statsociete']}",
            ParagraphStyle(
                'CompanyDetails',
                parent=styles['Normal'],
                fontSize=7,
        ...
        company_data = [[company_name], [company_details]]
        company_table = Table(company_data, colWidths=[company_width - 2*mm])
        ...
        title_width = operation_width * 0.65
        info_width = operation_width * 0.35
        ...
        operation_info = Paragraph(
            ...
            f"<b>Date :</b> {date_operation}<br/>"
            f"<b>Heure :</b> {datetime.now().strftime('%H:%M')}<br/>"
        ...
        operation_table = Table(operation_data, colWidths=[title_width, info_width])
        ...
        if table_data:
            ...
            table_rows = [list(columns)]
            table_rows.extend(data_rows)
            
            # Largeur du tableau: 100% (adapté à la page)
            col_width = page_width_usable / len(columns)
            col_widths = [col_width] * len(columns)
            
            articles_table = Table(table_rows, colWidths=col_widths, repeatRows=1)
            
            # Style du tableau: bordure uniquement en-tête et contour
            articles_table.setStyle(TableStyle([
                # En-tête avec bordure complète
                ...
                ('BORDER', (0, 0), (-1, 0), 1, self.COLOR_BORDER),
                
                # Données: pas de grille, seulement contour
                ('FONTSIZE', (0, 1), (-1, -1), 8),
                ('ALIGN', (0, 0), (1, -1), 'LEFT'),
                ...
                ('LINEBELOW', (0, -1), (-1, -1), 1, self.COLOR_BORDER),
                ('LINEABOVE', (0, 0), (-1, 0), 1, self.COLOR_BORDER),
                ('LINELEFT', (0, 0), (0, -1), 1, self.COLOR_BORDER),
                ('LINERIGHT', (-1, 0), (-1, -1), 1, self.COLOR_BORDER),
            ]))
            
            elements.append(articles_table)
            elements.append(Spacer(1, 3*mm))
        
        # ========== 4. DESCRIPTION SOULIGNÉE ==========
        if description:
            desc_line = Paragraph(
                f"<u>Description :</u> {description}",
                ParagraphStyle(
                    'Description',
                    parent=styles['Normal'],
                    fontSize=8,
        ...
        sig_left_2 = Paragraph(responsable_2, ParagraphStyle(
            'SigLabel2',
            parent=styles['Normal'],
            fontSize=8,
            alignment=TA_CENTER
        ...
        sig_right = Paragraph("Responsable", ParagraphStyle(
            'SigLabelRight',
            parent=styles['Normal'],
            fontSize=8,
            alignment=TA_CENTER
        ...
        sig_container.setStyle(TableStyle([
            ...
            ('LINEABOVE', (0, 0), (0, 0), 1, self.COLOR_BORDER),
            ('LINEABOVE', (2, 0), (2, 0), 1, self.COLOR_BORDER),
        ]))
